...
class SeqGraphAlignment(object):
	...
	def alignStringToGraphFast(self):
		...
		for i, node in enumerate(ni()):
			...
			bestdelete = np.zeros((l2), dtype=np.int32)+predecessors[0]+1

			matchpoints = self.matchscoreVec(gbase, seqvec)
			matchscore = scores[predecessors[0]+1, 0:-1] + matchpoints
			bestmatch = np.zeros((l2), dtype=np.int32)+predecessors[0]+1
	...
	def initializeDynamicProgrammingData(self):
		...
		# matrix
		scores = np.zeros((l1+1, l2+1), dtype=np.int32)
		...
		backStrIdx = np.zeros((l1+1, l2+1), dtype=np.int32)
		backGrphIdx = np.zeros((l1+1, l2+1), dtype=np.int32)
