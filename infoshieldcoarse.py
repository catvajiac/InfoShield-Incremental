...
class InfoShieldCoarse():
    def __init__(self, filename: str, doc_id_header='', doc_text_header='', num_phrases=10):
        ...
        # handle batching
        if 'date_posted' not in self.data.columns:
            print('Please include column name "date_posted"')
            exit(1)

        self.data['date_posted'] = pandas.to_datetime(self.data.date_posted, format='%m-%d-%Y')
		# create batch numbers column: finds index of each batch according to month, then adds the corresponding number to each ad_id
        self.data['batch_num'] = reduce(lambda l1, l2: l1 + l2,
                                  [[i]*len(subdf) for i, (_, subdf) in enumerate(self.data.groupby(pandas.Grouper(key='date_posted', freq='ME')))])


        self.num_ads = len(self.data.index)
        self.cluster_graph = nx.Graph()

    def setup_tfidf(self, batch_num):
        # setup tfidf
        ...
        self.length = Counter()
        self.data.loc[self.data.batch_num <= batch_num, self.description] = self.data[self.description].apply(filter_text)
    ...
    def clustering(self):
        ...
        index = 0
        for batch, subdf in self.data.groupby('batch_num'):
            self.setup_tfidf(batch)
